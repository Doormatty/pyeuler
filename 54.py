...
#            if len(set(hand).difference(set(card)))==1:
#                xof.append(card,set(hand).difference(set(card)))
#                highcard=card
...
#            else:
            ...
            if handval==1:
                ...
                handval=3
            xof.append(card)
            #highcard=max(set(hand).difference(set(card)))
            highcard=max(values)
            break
        ...
        if values.count(card)==2 and card not in xof:
            ...
            if handval==1:
                ...
                handval=1
            #highcard=max(set(hand).difference({card}))
            highcard=max(values)
          #  print ('Highcard is',highcard)
            #print ('xof is',xof)
